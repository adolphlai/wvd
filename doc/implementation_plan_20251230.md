...
- **`chest_auto` (自動寶箱)**:
    ...
    - 點擊後進入監控循環。
- **`gohome` (自動撤離 - 內部機制)**:
    - **僅作為內部 Fallback 機制**：用戶**無法**在 `quest.json` 中配置此目標。
    - **觸發條件**: 當移動超時（軟超時 60s）或 Resume 多次失敗時，腳本**自動**切換至 `gohome` 模式。
...
4.  **Resume / Chest_Resume 處理**:
    - **`resume` (一般移動恢復)**:
        - 在 `position`/`harken` 移動過程中偵測。
        - **觸發條件**: 畫面靜止且出現 `resume` 按鈕。
        - **重試邏輯**: 嘗試點擊 `resume` 最多 5 次。
        - **Fallback**: 若多次點擊無效，讓循環繼續跑直到觸發 Soft Timeout 轉為 `gohome`。
    - **`chest_resume` (自動寶箱恢復)**:
        - 在 `chest_auto` 移動過程中偵測。
        - **觸發條件**: 每 5 秒主動檢查 `chest_auto` 按鈕是否出現。
        - **行為**: 若按鈕出現（代表移動暫停），立即點擊繼續尋找下一個寶箱。
        - **結束條件**: 偵測到 `notresure`（無寶箱）→ 切換下一目標。
...
#### 重啟與初始化判斷邏輯
1.  **重啟後 (`_RESTART_OPEN_MAP_PENDING`)**: 跳過 Resume，直接嘗試打開地圖。
    - **成功打開地圖**: 正常導航。
    - **失敗（偵測到 `visibliityistoopoor` 暴風雪/能見度低）**: **直接觸發 GoHome**，這是暴風雪中唯一可離開的手段。
