...
[📝 GUI 編輯器使用說明](doc/EDITOR_USAGE.md) | [📖 自定義角色與技能說明](Custom_Assets_Guide.md) | [📜 quest.json 編輯指南](resources/quest/QUEST_EDIT_GUIDE.md)
...
### 🗺️ 智能地城刷怪
- 自動移動、尋路、戰鬥、開寶箱的完整循環
- **pyscrcpy 串流技術**：截圖延遲從 ~600ms 降至 ~1ms（效能提升 600 倍）
- 移動邏輯重構：GoHome 撤離機制、Resume 判定優化、轉向解卡
- 地城支援(狼洞1F|2F，深雪六樓自動寶箱，深雪教會區打怪)

### 🔄 自動重啟與異常處理
- 遊戲閃退、網路故障、卡住等情況自動重啟
- 畫面靜止檢測 + 超時保護（30 秒軟超時、60 秒硬超時）
- ADB 異常時自動重連/重啟模擬器
- 原地旋轉 BUG 自動檢測與修正
- **Harken 避讓移動**：重啟後透過小地圖偵測 Harken 位置與角色方向，嘗試寺方向移動，並避免往傳送點移動導致誤觸回城

### ⚔️ 戰鬥系統
- **第N戰個別配置**：可為最多 6 個角色設定技能，每個角色支援最多 10 組「第N戰」個別技能配置
	- 可辨識角色來執行對應技能，超出設定範圍的戰鬥自動沿用最後一組
- **打王模式**：在 quest.json 中標記打王步驟，自動切換至指定技能預設進行 Boss 戰
	- 支援 10 組技能預設，每組可為不同角色的每次行動設定不同技能
	- 戰鬥結束後自動恢復一般戰鬥模式
- **自行添加技能**：將技能截圖放入 `resources/images/spellskill/[類別]/` 即可
  - 支援類別：普攻、防禦、輔助、單體、橫排、全體、秘術、群控
  - 同時相容一般截圖與去背透明圖片（Alpha 通道），去背圖片自動啟用 Masked CCOEFF 比對，提升不同背景下的識別準確度
- **N 戰後自動**：配合遊戲內自動技能使用（完全自動/1場後/2場後/完全手動）
- **SP/MP 不足處理**：自動嘗試其他技能，若都不行則使用普攻
...
- 自動補給消耗品
- 自動整理背包並存入倉庫
	- 支援自行將物品圖片存至 `\resources\images\Organize`
- 可設定休息間隔次數

### 🩹 恢復設定
- 跳過戰後恢復 / 跳過開箱後恢復
- **低血量恢復**：移動時偵測到低血量 (紅色 10%-20%) 自動觸發恢復
- **異常狀態恢復**：支援中毒、劇毒、石化、麻痺、詛咒、恐懼、封技的自動偵測與恢復
- **麻痺/封技智慧重置**：偵測到麻痺或封技後，恢復完成時自動重置戰鬥計數器，避免因被迫普攻導致之後自動使用普攻。

### 📊 監控與日誌
- GUI 即時監控面板（當前狀態、效率統計）
- 詳細日誌記錄（含截圖）
- 戰鬥/寶箱計數與效率統計
- GUI 分頁重構與繁體化
- **血量偵測顯示**：地城移動時即時顯示角色血量狀態

### 📷 角色擷取功能
- **頭像截圖**：自動擷取角色頭像，支援自訂檔名
- 儲存至 `resources/images/character/`，下次啟動自動讀取
- 可用於角色技能設定中的角色識別

### 🛠️ Quest 編輯器 (Editor-Lite)
- **視覺化路線編輯**：透過桌面應用圖形化編輯 `quest.json`，支援地城路線（`_TARGETINFOLIST`）與回城步驟（`_EOT`）兩大分頁
- **即時模擬器連動**：可連接模擬器畫面，點擊畫面自動擷取座標、框選區域直接截圖存為識別模板
- **方向滑動捷徑**：內建左上/右上/左下/右下方向按鈕，點擊即發送 ADB 滑動指令，即時調試路線
- **動作類型完整支援**：position（座標移動）、stair/minimap_stair（樓層切換）、harken（傳送點）、chest_auto（自動寶箱）、press（圖片點擊）
- **任務管理**：建立、複製、重新命名、刪除任務；步驟支援拖拉排序與即時測試執行

---

## 📋 版本更新記錄

### v1.12.0 最新穩定版 (2026-01-28)

29. **【重大更新】技能配置重構與效能優化**
    - **個別配置**：「首戰/二戰後」擴展為最多 10 組「第N戰」個別配置，超出對應場次自動沿用最後一組。
    - **效能提升**：優化開寶箱與戰鬥偵測邏輯，掛機過程更加順暢。
    - **介面優化**：GUI 簡化為每角色 1 行，修復重疊問題並新增「打王模式」說明。
    - **編輯器增強**：完善 README 中的 Quest 編輯器功能介紹。
    - **向下相容**：舊版設定啟動時自動遷移，無需手動修改。

### v1.11.32 (2026-01-27)

28. **【系統優化】傳送避讓與牆壁偵測強化**
    - 優化哈肯 (Harken) 傳送點附近的避讓移動邏輯，解決重啟後可能發生的打轉問題。
    - 恢復穩定的牆壁偵測基準參數，大幅提升自動導航的準確性。
    - 修復 DungeonMover 全域硬超時計時器重置問題與戰鬥 2 倍速自動檢查。

### v1.11.31 任務擴展與崩潰自癒 (2026-01-20)

27. **【新功能】深雪 4F 忍者刷機與守護進程**
    - 新增深雪 4F 任務完整路徑優化。
    - 升級全局監控機制，支援遊戲崩潰後自動恢復導航進度。
    - 強化登入畫面、地圖跳轉與村莊地標的自動識別能力。

### v1.11.30 穩定性重大升級 (2026-01-19)

26. **【修復】解決重啟「無限找村莊」死循環**
    - 修正了重啟後可能陷入「無限找地標」的問題，優化視窗渲染等待時間。
    - 新增重啟後自動位置校正，智能判斷村莊與地城狀態。

### v1.11.25 透明遮罩比對技術 (2026-01-19)

25. **【技術提升】全面支援透明遮罩技能比對**
    - 引進 Masked CCOEFF 匹配算法，解決半透明技能圖示在不同背景下的識別失敗問題。
    - 新增大量角色（如：切羽、凜音）與技能資源圖片。

### v1.11.10 異常狀態自動恢復
    - 新增 **中毒、劇毒、石化、麻痺、詛咒、恐懼** 的自動偵測與恢復功能
    - 採用高精準度的像素與顏色分析算法，有效區分外觀相似的異常狀態（如劇毒/恐懼/詛咒）
    - 針對「恐懼」狀態開發了專用的 Pixel Diff 檢測邏輯，確保零誤判
    - 可於「進階設定」中獨立開關各狀態的恢復功能

### v1.11.0 重大更新

22. **【重大優化】地城移動邏輯重構** - 解決原地轉圈與卡死問題
    - **GoHome 撤離機制**：當移動超時或遭遇未知卡死時，優先嘗試使用 GoHome 功能撤離
    - **Resume 判定優化**：更準確的 Resume 按鈕點擊判定，避免畫面誤判
    - **轉向解卡優化**：偵測到長時間靜止時，自動嘗試滑動畫面解除卡角狀態

23. **【效能提升】寶箱開啟邏輯 (StateChest) 重寫**
    - 邏輯簡化：重構 `StateChest` 循環，移除冗餘判斷
    - 突發連點 (Burst Click)：大幅加快對話跳過與開箱速度
    - 分頻檢測：降低異常狀態與死亡檢測頻率
...
21. **【重大更新】pyscrcpy 串流截圖整合** - 截圖效能提升 600 倍
    - 整合 pyscrcpy 視頻串流技術，截圖延遲從 ~600ms 降至 ~1ms
    - 自動降級：若 pyscrcpy 不可用，自動退回傳統 ADB 截圖
    - 移動檢測更靈敏，可更早偵測到角色抵達目的地

### v1.9.30 新增功能

18. **Harken 樓層選擇功能** - 支援選擇 Harken 傳送的目標樓層

19. **Resume 失敗樓梯判定優化** - 偵測到樓梯目標時判定為換樓成功

20. **【重要修復】地圖目標重複處理問題** - 修復 position/stair 目標被執行兩次的 BUG

### v1.9.29 新增功能

17. **整理背包功能** - 自動整理背包並存入倉庫

### v1.9.28 新增功能

14. **GUI 分頁重構與繁體化** - 介面美化與易用性提升

15. **原地旋轉 BUG 檢測與修正** - 解決角色在某些情況下無限轉圈的問題
    - 新增自動檢測邏輯，當判定為原地旋轉時，自動進行四方向移動以嘗試脫困

16. **移動超時 60 秒自動重啟** - 防止原地旋轉 BUG 的最終保護機制
    - 當角色處於移動狀態超過 60 秒仍未到達目的地時，自動重啟遊戲
    - 這是針對四方向移動無法解決的極端情況的備用方案

### v1.9.27 新增功能

13. **分離重啟後/返回後首戰設定** - 更精細的控制
    - GUI 新增兩個獨立選項：「重啟後首戰」與「返回後首戰」
    - 可分別控制遊戲重啟和從旅館返回時的強力技能觸發

### v1.9.27 Bug 修復

- **修正強力技能觸發邏輯** - 只在遊戲真正重啟時才觸發，正常啟動腳本不再誤觸發
- **修正計數器重置時機** - 計數器只在 `restartGame` 中重置，而不是每次地城循環

### v1.9.26 新增功能

10. **Resume 按鈕優化** - 減少地圖操作，提高效率
    - 可在 GUI 中開關此功能
    - 戰鬥/寶箱後自動點擊 Resume 繼續尋路
    - 檢測 `routenotfound` 判斷是否已到達目的地
    - 畫面靜止 3 次觸發回城邏輯

11. **重啟/返回後首戰強力單體技能** 
    - 可在 GUI 中開關此功能
    - 重啟遊戲後或從村莊返回後，第一場以及第二場戰鬥整場使用強力單體技能
    - 自動關閉自動戰鬥模式以釋放技能
    - 戰鬥結束後恢復用戶設定的戰鬥邏輯

12. **防止原地轉圈** - 重啟後自動左右平移
    - 修復了重啟後 `_STEPAFTERRESTART` 標誌未重置的 Bug

### v1.9.26 Bug 修復

- **修復 gohome 回城邏輯** - 解決無法正確檢測已回到村莊的問題
- **修復自動戰鬥按鈕連續點擊問題** - 防止按鈕被點擊兩次導致開啟後又關閉
...
6. **修復進程不退出問題** - 修復使用 --noconsole 後關閉 GUI 進程仍留在後台的問題
   - 檔案：`src/main.py` 添加 on_closing() 方法，設置所有線程為 daemon
   - 檔案：`src/gui.py` 修改 WM_DELETE_WINDOW 協議處理器
   - 修改：關閉窗口時正確停止任務線程、日誌監聽器並清理資源
   - 修改：所有任務線程設置為 daemon=True，主程序退出時自動終止
   - 修改：使用 os._exit(0) 強制終止進程，確保完全退出
...
7. **修復截圖函數無限阻塞問題** - 解決運行中程式卡死在截圖操作的問題
   ...
   - 修改：ADB screencap() 調用添加 10 秒超時機制，防止永久阻塞
   - 效果：截圖失敗時會記錄重試次數並最終拋出異常，避免永久卡住

8. **修復快速啟動/停止導致無響應問題** - 解決快速點擊啟動後停止或關閉窗口時程序卡死的問題
   - 檔案：`src/script.py` CheckRestartConnectADB() 和 Farm() 函數
   - 問題：ADB 連接重試最多 20 次，每次多個 2 秒 sleep，沒有檢查停止信號
   - 修改：CheckRestartConnectADB() 在每次重試前和所有 sleep 中檢查停止信號
   - 修改：所有 2 秒 sleep 改為 4 次 0.5 秒，每次檢查停止信號
   - 修改：Farm() 添加異常處理和停止信號檢查，ADB 失敗時調用 callback
   - 效果：快速啟動/停止時最多 0.5 秒響應，不再卡在 ADB 連接過程中

9. **本地打包腳本** - 使用繁體中文編碼的打包腳本，避免簡體版本的編碼問題
   - 檔案：`localpack.bat`
