...
**版本基礎：** 上游原作者 arnold2957/wvd v1.9.30

### v1.11.0 重大更新

22. **【重大優化】地城移動邏輯重構** - 解決原地轉圈與卡死問題
    - **GoHome 撤離機制**：當移動超時或遭遇未知卡死時，優先嘗試使用 GoHome 功能撤離，而非直接重啟遊戲
    - **Resume 判定優化**：更準確的 Resume 按鈕點擊判定，避免畫面誤判
    - **轉向解卡優化**：偵測到長時間靜止時，自動嘗試滑動畫面以解除可能的卡角狀態

23. **【效能提升】寶箱開啟邏輯 (StateChest) 重寫**
    - **邏輯簡化**：重構 `StateChest` 循環，移除冗餘判斷
    - **突發連點 (Burst Click)**：在檢測空檔加入連點機制，大幅加快對話跳過與開箱速度
    - **分頻檢測**：降低異常狀態與死亡檢測的頻率，減少圖像識別負載

24. **【Bug 修復】戰鬥技能施放修正**
    - 修復了部分情況下（如無可用技能時）戰鬥邏輯卡死的問題
    - 優化了第二場戰鬥的技能判定順序
...
20. **【重要修復】地圖目標重複處理問題** - 修復 position/stair 目標被執行兩次的 BUG
    - 原問題：每個 position 目標都會滑動兩次（第一次點擊移動，第二次才發現「已抵達」）
    - 修復後：點擊移動後立即標記完成，每個目標只滑動一次
    - 大幅提升地城路線執行效率
