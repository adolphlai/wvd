...
- **視覺化路線編輯**：透過 Electron 桌面應用圖形化編輯 `quest.json`，支援地城路線（`_TARGETINFOLIST`）與回城步驟（`_EOT`）兩大分頁
- **即時模擬器連動**：WebSocket 連接模擬器畫面，點擊畫面自動擷取座標、框選區域直接截圖存為識別模板
...
29. **【新功能】技能配置支援第N戰個別配置**
    - 「首戰/二戰後」擴展為最多 10 組「第N戰」個別配置，超出範圍自動沿用最後一組。
    - GUI 簡化為每角色 1 行，透過「第N戰選擇」下拉選單切換戰鬥頁。
    - 舊版設定啟動時自動遷移，完全向下相容。
