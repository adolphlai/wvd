**新版本!**
==v1.11.11==
1. **DungeonMover 邏輯修復**：修復了 Resume 優化在多目標切換時的路徑殘留問題，確保切換目標後強制開啟地圖。
2. **目標列表持久化**：改進 `targetInfoList` 管理機制，防止在地城內因狀態切換導致導航進度被意外重置。
3. **新增導航屏障機制**：引入 `_RESTART_OPEN_MAP_PENDING` 標記，精確控制何時允許執行 Resume 優化。
4. **強化導航追蹤日誌**：所有導航目標的完成彈出（pop）現在都會顯示剩餘目標數，便於追蹤進度。
5. **架構文件深度更新**：詳盡文檔化了 `DungeonMover` 的監控優先級與 Chest/Stair 判定模式。
