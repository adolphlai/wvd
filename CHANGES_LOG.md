...
==v1.11.25==
1. **技能圖片去背工具**：
   - **[新增]** GUI 測試分頁新增「圖片去背工具」，支援自由選擇目錄並對 PNG 圖片進行智慧去背處理。
   - **[新增]** 技能圖片支援透明遮罩比對（Masked CCOEFF），大幅提升不同背景下的識別準確率。
   - **[技術]** 新增 `LoadImageWithAlpha` 函數，僅技能圖片使用 Alpha 通道讀取。
2. **穩定性與效能優化**：
   - **[修正]** 修復 OpenCV matchTemplate 因通道不匹配導致的程式崩潰。
   - **[修正]** 修復停止任務時無響應的問題，在戰鬥模板遍歷中加入停止信號檢查。
3. **介面簡化**：
   - **[移除]** 移除測試分頁中的「Inn 流程測試」與「小地圖樓梯偵測測試」區塊。
