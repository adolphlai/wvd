...
==v1.11.21==
1. **新功能**：
   - **Swipe 移動模式**：新增滑動移動功能，適用於部分需要精確方位控制的地城（如打王模式）。
   - **輔助技能支援**：技能類別新增「輔助」分類，支援指定目標（1~6 號位）。
   - **Debug 截圖選項**：測試分頁新增「debug截圖」開關，可控制異常狀態偵測時是否保存截圖（預設關閉）。
   - **新增角色與技能圖片**：凜音、土法異裝、愛麗絲異裝、矮肥短、米拉娜、艾莉賽；殘心、霧消、噴火擊、超級緩速等技能。
2. **Bug 修復**：
   - 修復模擬器崩潰後恢復機制失效的問題。
...
3. **穩定性提升**：
   - 優化 debug 截圖路徑，解決編譯版本下截圖存到臨時目錄的問題。
