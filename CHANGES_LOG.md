...
1. **技能配置重構：支援第N戰個別配置**：
   ...
   - 超出設定範圍的戰鬥自動沿用最後一組設定（例：設定了 3 頁，第 4 戰以後沿用第 3 頁）。
   - 打王模式與一般戰鬥共用相同資料結構，可為 Boss 每次行動設定不同技能。
2. **GUI 簡化**：
   - 每個角色從 2 行（首戰/二戰後）簡化為 **1 行**，透過「第N戰選擇」下拉選單切換不同戰鬥頁。
   - 新增「新增戰鬥頁」與「刪除最後一頁」按鈕，操作更直覺。
3. **向下相容**：
   - 舊版 `skill_first / skill_after` 格式會在啟動時自動遷移為新的 `battles[]` 格式，無需手動操作。
