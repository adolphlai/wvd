...
==v1.11.12==
1. **戰鬥系統重大更新**：
   - **普攻重構**：將普通攻擊改為動態目標判定（依據 `next` 按鈕偏移），大幅提升準確度。
   - **防禦功能**：新增「防禦」技能類別，支援直接施放邏輯。
   - **邏輯統一**：整合 SP/MP 不足時的自動普攻保底機制。
2. **自動化流程優化**：
   - **GitHub Release 優化**：自動提取日誌與補全說明文件打包。
3. **介面更新**：調整 GUI 標題與反饋連結。

==v1.11.11==
1. **DungeonMover 邏輯修復**：修復了 Resume 優化在多目標切換時的路徑殘留問題。
2. **導航自癒能力**：開啟地圖失敗時會自動補償偵測並點擊 Resume (AUTO)。
3. **目標列表持久化**：改進 `targetInfoList` 存儲機制，防止地城內導航進度重置。
