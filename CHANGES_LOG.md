...
(2026-01-28)
1. **技能配置重構：支援第N戰個別配置**：
   - 將原本固定的「首戰 / 二戰後」兩組技能設定，擴展為最多 **10 組**「第N戰」個別配置。
   - 超出設定範圍的戰鬥自動延用最後一組設定（例：設定了 3 頁，第 4 戰以後沿用第 3 頁）。
   - 打王模式與一般戰鬥共用相同資料結構，可為 Boss 每次行動設定不同技能。
2. **GUI 簡化與優化**：
   - 每個角色簡化為 **1 行**，透過下拉選單切換戰鬥頁，並修復了文字重疊問題。
   - 新增「新增戰鬥頁」按鈕，並加入「打王模式」專屬提示。
   - 戰鬥場次說明更完善（第 N 次行動 = 對應戰鬥頁設定）。
3. **執行效率大提升**：
   - 優化了開寶箱後的等待邏輯，大幅加快開箱速度。
   - 加速戰鬥介面的偵測與同步，提升整體掛機流暢度。
4. **地圖編輯器完善**：
   - 在 README 中補充了 Editor-Lite 的功能描述，並在 GUI 新增啟動說明。
5. **向下相容**：
   - 舊版設定啟動時自動遷移為新格式。
...
(2026-01-27)
1. **系統穩定性優化**：
   - 優化重啟後的移動避讓邏輯，有效防止在哈肯 (Harken) 傳送點附近發生重複打轉的情況。
   - 強化牆壁偵測的準確度，恢復核心偵測參數至穩定版本，提升自動導航的導航可靠性。
2. **問題修復**：
   - 修復地底移動時，因計時器未正確重置導致的超時誤判問題。
   - 修復戰鬥中 2 倍速狀態的自動檢查機制，確保每場戰鬥都能維持最高效率。

**歷史更新**
==v1.11.31==
(2026-01-20)
1. **任務優化**：新增深雪4F刷忍者流程，並優化導航路徑。
2. **崩潰偵測**：升級全局監控機制，大幅縮短遊戲崩潰後的重啟響應時間。
3. **智慧恢復**：新增重啟後狀態自動導回機制，能自動處理登入畫面與村莊導航。
4. **穩定性修正**：
   - 修正遊戲下載更新時的誤報重啟問題。
   - 修正計數器顯示不準確（遊戲崩潰與 ADB 重連）的問題。
   - 優化 ADB 重連穩定性與計時器重置邏輯。

**歷史更新**
==v1.11.30==
(2026-01-19)
1. **重啟穩定性重大升級**：
   - 解決了重啟後可能陷入「無限找村莊地標」死循環的問題。現在腳本會更精確地等待遊戲畫面真正加載完成。
   - 新增「自動位置校正」功能：重啟後會自動判斷目前是在村莊還是已經在地城裡，如果是「斷線重連」會自動接續任務，不再跑空路。

**歷史更新**
==v1.11.29==
(2026-01-19)
1. **多開模擬器支援**：
   - 支援指定特定編號的模擬器（如 0, 1, 2...），方便多開玩家精確控制。
   - 一般用戶預設使用主視窗，無需額外設定。
2. **重啟穩定性優化**：
   - 修正了遊戲重啟後，因為載入畫面的黑屏時間過長，導致腳本誤以為卡死而無限重啟的問題。
   - 現在會給予額外的 60 秒寬限時間讓遊戲完成載入。

==v1.11.28==
(2026-01-19)
1. **戰鬥判定修復**：
   - 修復了因為進出戰鬥太快或黑屏切換導致的誤判，現在會確保真正進入戰鬥畫面後才開始結算。
   - 修正了「自動開箱」後的導航問題，確保開完箱子後能正確打開地圖前往下個目標。
2. **穩定性優化**：
   - 優化了模擬器視窗的前台檢測機制，能更準確地知道遊戲是否在最上層。
   - 移除了戰鬥日誌中的除錯訊息，讓主要日誌更乾淨。
3. **重啟與監控邏輯重構**：
   - 完善了 `GameMonitor` 進程監控機制，每 2 秒檢查遊戲存活與前台狀態，崩潰時自動透過 `Sleep()` 觸發重啟。
   - 強化了 ADB 重連邏輯 (`ResetADBDevice`)，在模擬器重啟或斷線後能自動恢復 `pyscrcpy` 串流與遊戲進程。
   - 優化了 `RestartGame` 流程，連續崩潰 5 次會觸發模擬器強制重啟，並引入 `_IN_RESTART` 標記防止遞迴重啟。

**歷史更新**
==v1.11.27==
(2026-01-19)
1. **地城探險優化**：
   - 修復了連續刷地城時可能發生的「過門不入」問題，現在進入地下城的點擊動作更加準確且具備自動重試機制。
   - 同步優化了從村莊出發進入地城的穩定性，適應各種動畫延遲。
2. **戰鬥監控強化**：
   - 在戰鬥介面中新增了「紅字診斷功能」，當戰鬥發生異常中斷時，能更清楚地追蹤原因。
   - 解決了部分情況下戰鬥加載過快導致的誤判問題。
3. **系統清理**：
   - 清理了老舊的測試檔案與暫存資料，讓程式運行環境更簡潔。
...
1. **新增與優化任務**：
   - 新增「巨人」懸賞任務完整流程支援。
   - 優化「蠍女+牛頭人」領獎流程，解決點擊過快導致獎勵視窗誤關的問題。
2. **穩定性修復**：
   - 大幅提升停止腳本時的響應速度，解決原本可能發生的卡死問題。
   - 優化地城移動判定，修復 Quest 模式下可能發生的無限 Resume 重試。
   - 修復程式關閉時背景程序可能殘留的問題。

**歷史更新**
==v1.11.25==
1. **技能圖片工具與資源更新**：
   - **[新增]** GUI 測試分頁新增「圖片去背工具」，支援智慧去背並自動替換。
   - **[新增]** 技能圖片支援透明遮罩比對（Masked CCOEFF），並更新多張技能與角色頭像資源：
     - **頭像**：新增「切羽」。
     - **技能**：更新「死死連葬」、「強襲」、「霧消」、「萬紫千紅」、「緩速攻擊」。
   - **[技術]** 新增 `LoadImageWithAlpha` 專門處理透明度載入。
2. **穩定性與效能優化**：
   - **[修正]** 修復 OpenCV matchTemplate 在非技能圖片上的相容性問題（通道不匹配）。
   - **[修正]** 加強任務停止響應，在戰鬥模板遍歷中確保能快速退出。
3. **介面調整**：
   - **[優化]** 去背工具資料夾選擇預設指向 `spellskill` 目錄。
   - **[移除]** 清理測試分頁中過時的 Inn 與小地圖測試區塊。

**歷史更新**
==v1.11.24==
1. **「美麗礦石的真相」跳躍與任務優化**：
   - **[新增]** 在時間跳躍 (`CursedWheelTimeLeap`) 中支持「美麗礦石的真相」節點。
   - **[新增]** 多目標門檻識別模式：導航時同時辨識目標與環境標籤（深雪邊境、昔日之影），大幅提升識別成功率。
   - **[優化]** 蠍女任務流程升級：開啟跳躍後自動跳過返回要塞與前往王城的導航步驟，節省約 10-15 秒。
   - **[GUI]** 進階設定分頁新增手動開關選項。
2. **穩定性修復**：
   - **[修正]** 修復「連續刷地城」在跳過回城時，若遇到特定的轉場對話框可能觸發 `NoneType` 迭代錯誤導致腳本崩潰的問題。
   - **[修正]** 修復當「嘗試調整因果」未勾選時，腳本仍會主動點擊伏擊或忽略對話框的問題。
   - 現在善惡調整完全受 `ACTIVE_CSC` 設置控制，確保符合使用者手動意圖。

**歷史更新**
==v1.11.23==
1. **導航穩定性修復**：
   - **[修正]** 修復 `quest.json` 設置 `harken` 或 `minimap_stair` 目標時，因屬性定義缺失（floorImage）導致的腳本報錯崩潰。
   - 統一 `TargetInfo` 屬性映射，提升導航相容性。

**歷史更新**
==v1.11.22==
1. **穩定性提升與容錯優化**：
   - 最大重啟次數從 50 增加至 100，提升長時間掛機穩定性。
   - 角色頁面 trait 偵測次數從 5 增加至 10，適應低效能環境。
   - 遊戲重啟時增加 5 秒固定等待，避免模擬器尚未就緒。
2. **移動與進度維護修復**：
   - **[修正]** 修復重啟後 `DungeonMover` 硬超時計時器未重置導致連環重啟的問題。
   - **[優化]** 簡化防止轉圈邏輯，註解前進/後退動作，僅保留左右平移（省下約 2 秒）。
3. **功能性修復**：
   - **[修正]** `AUTO` 移動過程中加入「伏擊/忽略」善惡對話框識別，依據 `_KARMAADJUST` 自動點擊。
   - **[Beta]** 改進 `resume` 和 `chest_auto` 的全域重試與超時 fallback 機制。

**歷史更新**
==v1.11.21==
1. **打王模式**：
   - 新增獨立打王模式，目前適用於「**[打王]深雪教會區**」任務。
   - 預設使用**第 10 組技能配置**作為打王專用技能。
   - 使用方式：先在技能分頁編輯好第 10 組打王專用技能。
   - 打王時，「首戰/二戰」配置自動轉換為「第一輪/第二輪」技能施放順序。
2. **輔助技能支援**：
   - 技能類別新增「輔助」分類，可對隊友施放。
   - 支援指定目標：1~6 號位隊員。
   - 使用方式：將技能圖片放在 `spellskill/輔助` 資料夾下。
3. **Debug 截圖選項**：
   - 測試分頁新增「debug截圖」開關（預設關閉）。
   - 開啟後可保存異常狀態偵測時的截圖至 `debug_screens/` 資料夾。
4. **新增角色與技能圖片**：
   - 角色：凜音、土法異裝、愛麗絲異裝、矮肥短、米拉娜、艾莉賽
   - 技能：殘心、霧消、噴火擊、超級緩速、扇形攻擊、法系霧消、反叛祈禱
5. **Bug 修復**：
   - 修復模擬器崩潰後恢復機制失效的問題。
   - 增強 StateEoT 與復活機制的異常處理。
   - 修復 GoHome 模式靜止判定導致的無限循環。
   - 將戰鬥、開箱、因果調整加入網路波動（retry）處理。

**歷史更新**
==v1.11.20==
1. **硬超時修復**：
   - 修復戰鬥/寶箱硬超時檢測（400秒）永遠不觸發的問題。
   - 將檢測邏輯移至迴圈開頭，確保遊戲卡住時能正確重啟。
2. **日誌優化**：
   - 硬超時計時器 debug 日誌改為每 10 秒顯示一次，避免日誌過於頻繁。

**歷史更新**
==v1.11.19==
1. **恢復流程優化**：
   - 優化 trait 偵測機制，點擊角色後連續偵測最多 5 次（每次間隔 0.5 秒）。
   - 適應慢機器角色詳情頁面打開較慢的情況，偵測到即繼續，避免不必要的等待。

**歷史更新**
==v1.11.18==
1. **AE 觸發間隔修復**：
   - 修正連續刷地城（跳過回城）時，計數器使用錯誤導致每次都被判定為「第一場」的問題。
   - 現在使用 `_DUNGEON_REPEAT_COUNT` 作為間隔判斷基準，確保第 2~N 場正確跳過黑屏並啟用自動戰鬥。
2. **Quest 編輯器穩定性**：
   - 修復停止伺服器時無回應的問題。
3. **其他小修復**。
...
==v1.11.17==
1. **Quest 編輯器功能大躍進**：
   - **滑動指令捷徑**：新增「左上、右上、左下、右下」按鈕，點擊即透過 WebSocket 發送 ADB 指令，實現即時方位調試。
   - **格式持久化優化**：新增「緊湊 JSON 格式化」邏輯，確保存檔後 `quest.json` 保持簡潔。
   - **部署穩定性提升**：打包腳本加入資產清理與同步，解決瀏覽器快取舊代碼的問題。
   - **UI 與預設值微調**：新增寶箱項目不再預設帶有座標；優化按鈕視覺反饋。
2. **開發與環境優化**：
   - **回退機制強化**：優化 `/revert_release` 工作流，確保標籤與代碼版號同步回退。
   - **CI/CD 整合**：正式將 `editor-lite` 打包流程整合至 GitHub Actions。
   - **文檔同步**：更新編輯器使用說明。

**歷史更新**
==v1.11.16==
1. **連續刷地城功能修復**：
   - **跳過回城後立即退出**：修復連續刷地城時跳過回城後腳本立即退出地城的問題。新增目標列表重置機制與 Resume 優化跳過邏輯。
   - **計數器準確性**：修復 `IdentifyState` 遞歸調用導致連續刷地城計數器重複累加的問題。
   - **狀態標誌完善**：新增 `_RESET_TARGETS_PENDING` 標誌，確保跳過回城後正確重新初始化目標列表。

**歷史更新**
==v1.11.15==

1. **自動戰鬥邏輯優化**：
   - **重啟策略修復**：徹底修復重啟後 `_RESTART_SKIP_INTERVAL_THIS_DUNGEON` 旗標因重置順序不當而失效的問題。現在重啟後的第一場戰鬥能正確遵循 UI 設定的模式。
   - **狀態一致性**：新增重啟後跳過間隔判斷（AE_CASTER_INTERVAL）的機制。
2. **文檔完善**：
   - 新增「自動戰鬥模式狀態控制分析」技術文件。

**歷史更新**
==v1.11.14==
1. **功能新增與整合**：
   - **寶箱導航整合**：將 `chest` 目標類型正式整合至 `DungeonMover` 移動架構，支援地圖自動旋轉搜尋與精確 ROI 判定。
   - **系統核心優化**：新增全域停止信號處理機制，確保腳本在手動停止時能更優雅且迅速地響應。
   - **任務編輯器**：核心代碼新增任務編輯器支持。
2. **穩定性提升**：
   - **修復遞迴錯誤**：重構 `IdentifyState` 機制，將遞迴調用改為迭代循環，徹底解決長時間運行後可能出現的「最大遞迴深度超出」崩潰問題。
   - **異常檢測優化**：精確調整多種異常狀態（石化、恐懼、封技等）的高級檢測算法，降低誤判率。
   - **代碼架構重構**：移除冗餘的 stoppable 裝飾器與過時的 flag，簡化狀態機邏輯。

**歷史更新**
==v1.11.13==
1. **Bug 修復**：
   - 修復截圖功能在 Windows 下中文檔名無法儲存的問題（改用 PIL 替代 cv2.imwrite）。
   - 修復重啟流程導致的無限循環問題。
2. **易用性優化**：
   - 截圖按鈕文字簡化：「擷取截圖」→「畫面截圖」、「擷取角色(ROI)」→「頭像截圖」。

==v1.11.12==
1. **戰鬥系統更新**：
   - **普攻重構**：將普通攻擊改為動態目標判定（依據 `next` 按鈕偏移），大幅提升在不同敵陣下的準確率。
   - **防禦功能**：新增「防禦」技能類別，支援直接施放邏輯（點擊圖標後立即結束行動）。
   - **狀態一致性**：整合 SP/MP 不足時的保底普攻調用邏輯。
2. **易用性提升**：
   - **更新日誌自動顯示**：程式升級後首次啟動將自動彈出此視窗，方便查看變動訊息。
3. **系統與工程優化**：
   - **GitHub Release 自動化**：發布流程現在會自動從此日誌提取文字，且下載的壓縮包會完整包含 README 與 Guide 等文件。

==v1.11.11==
1. **DungeonMover 邏輯修復**：修復了 Resume 優化在多目標切換時的路徑殘留問題，並新增「破窗清理」機制解決結算視窗遮擋問題。
2. **導航自癒能力**：開啟地圖失敗時會自動補償偵測並點擊 Resume (AUTO)，解決視覺死角導致的 GoHome 循環。
3. **目標列表持久化**：改進 `targetInfoList` 存儲機制，防止在地城內因狀態切換導致導航進度被意外重置。
4. **新增導航屏障機制**：精確控制 `_RESTART_OPEN_MAP_PENDING` 標記，確保目標切換後必須通過開啟地圖啟動。
5. **強化導航追蹤日誌**：所有導航目標的彈出現在都會顯示剩餘目標數，並同步更新了架構重構文檔。

==v1.11.10==
1. 優化 DungeonMover 架構：新增開啟地圖失敗後的 1 秒延遲與黑幕偵測，提升穩定性。
2. 新增「封技 (SkillLock)」狀態恢復支持。
3. 將「恐懼」更名為「寶箱恐懼」，並優化偵測日誌輸出。
4. GUI 狀態恢復區塊新增自動換行（每行最多 5 個）。
5. 同步更新偵測審計工具與除錯腳本。
