...
==v1.11.18==
1. **Quest 編輯器優化**：
   - **格式持久化**：新增「緊湊 JSON 格式化」邏輯，確保編輯器存檔後 `quest.json` 中的座標、指令清單能保持在一行顯示，解決格式過於分散的問題。
   - **資料修復**：全面修復並優化 `resources/quest/quest.json` 為緊湊格式，還原原本緊湊的展示風格。
2. **架構微調**：
   - 重構 `EditorWebSocketServer` 的格式化輔助函數至模組級別，提升代碼健壯性。

**歷史更新**
==v1.11.17==
1. **CI/CD 與 編輯器整合**：
   - **CI/CD 流程整合**：將 `editor-lite` 編輯器打包流程整合至 GitHub Actions，自動編譯前端並打包 `DungeonEditor.exe`，此後發布版本將包含編輯器。
   - **編輯器功能增強**：
     - **路徑修正**：修復打包後資源路徑錯誤、截圖路徑前綴問題、以及 `click_image` 搜尋路徑問題。
     - **介面優化**：Harken 搜尋範圍框改為綠色並更名標籤；修正座標輸入框佈局；Modal 介面全面中文化。
     - **功能改進**：新增腳本複製功能；移除內建示範資料。
     - **穩定性修復**：修復打包版本缺少 `websockets` 導致的崩潰問題，並新增依賴檢查。
2. **核心優化**：
   - **停止信號響應**：優化 `script.py` 中的等待邏輯（如模擬器啟動、寬限期），使用分段 sleep 確保能迅速響應停止信號。
