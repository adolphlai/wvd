**新版本!**
==v1.11.11==
1. **戰鬥邏輯優化**：重構普攻判定為動態目標鎖定，與單體技能一致；統一 SP/MP 不足時的保底調用。
2. **新增防禦技能**：新增「防禦」類別支援直接施放（點擊即結束行動），並優化相關資源目錄。
3. **GitHub Release 優化**：更新打包工作流，支援自動提取日誌與納入 README 等說明文件。
4. **DungeonMover 邏輯修復**：修復了 Resume 優化在多目標切換時的路徑殘留問題，並新增「破窗清理」機制解決結算視窗遮擋問題。
5. **導航自癒能力**：開啟地圖失敗時會自動補償偵測並點擊 Resume (AUTO)，解決視覺死角導致的 GoHome 循環。
6. **目標列表持久化**：改進 `targetInfoList` 存儲機制，防止在地城內因狀態切換導致導航進度被意外重置。
