...
==v1.11.22==
1. **穩定性提升**：
   - 最大重啟次數從 50 增加至 100，提高長時間運行穩定性。
   - 角色頁面 trait 偵測次數從 5 增加至 10，適應慢機器開啟較慢的情況。
   - 遊戲重啟時增加 5 秒等待，確保模擬器完全就緒。
2. **善惡選擇修復**：
   - 修復 DungeonMover 移動時遇到善惡對話框會誤點的問題。
   - 現在會根據 `_KARMAADJUST` 設定自動選擇伏擊/忽略。
3. **[BETA] 改進 resume 和 chest_auto 的 fallback 機制**：
   - 新增全域重試與超時機制。
