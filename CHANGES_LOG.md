...
==v1.11.11==
1. **戰鬥系統更新**：
   - **普攻重構**：將普通攻擊改為動態目標判定（依據 `next` 按鈕偏移），提升準確度。
   - **防禦功能**：新增「防禦」技能類別，支援直接施放邏輯。
   - **日誌顯示**：新增啟動時自動展示更新日誌的功能。
2. **自動化優化**：優化 GitHub Action 打包流程，自動提取更新日誌。
3. **導航與修復**：
   - 修復了 Resume 優化在多目標切換時的路徑殘留問題。
   - 開啟地圖失敗時會自動補償偵測並點擊 Resume (AUTO)。
   - 改進 `targetInfoList` 存儲機制，防止地城內導航進度重置。
   - 精確控制 `_RESTART_OPEN_MAP_PENDING` 標記。
4. **介面更新**：調整 GUI 標題、反饋連結與目標追蹤日誌顯示。
