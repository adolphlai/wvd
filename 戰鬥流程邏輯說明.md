...
│     └─ 無施法序列 → 進入步驟 3
│
├─ 3. ★ 重啟後第一次戰鬥 (_FIRST_COMBAT_AFTER_RESTART = True)
│     │  【優先於自動戰鬥！】
│     │
...
│     └─ 功能已關閉 → 進入步驟 5
│
├─ 4. ★ 村莊返回後第一次戰鬥 (_FIRST_COMBAT_AFTER_INN = True)
│     │  【優先於自動戰鬥！】
│     │
│     ├─ 功能已啟用 (_FORCE_PHYSICAL_FIRST_COMBAT = True)
│     │     ├─ 關閉自動戰鬥（如果已開啟）
...
> **觸發條件**：重啟後第一次戰鬥 或 村莊返回後第一次戰鬥

```
【強力單體技能處理】
│
├─ 檢查功能開關 (_FORCE_PHYSICAL_FIRST_COMBAT)
│     │
│     └─ 已關閉 → 記錄日誌 → 返回 False
│
├─ 記錄日誌：{原因}，强制使用强力单体技能
...
| `_FORCE_PHYSICAL_FIRST_COMBAT` | 重啟/返回後首戰使用強力單體技能開關 |
| `_FIRST_COMBAT_AFTER_RESTART` | 重啟後第一次戰鬥標誌 |
| `_FIRST_COMBAT_AFTER_INN` | 村莊返回後第一次戰鬥標誌 |
...
| `_FIRST_COMBAT_AFTER_INN` | bool | False | 村莊返回後第一次戰鬥標誌 |
