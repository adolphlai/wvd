...
def main():
    path = r"D:\Project\wvd\detect\中毒"
    icon = load_image(os.path.join(path, "Poison_icon.png"))
    if icon is None: return
    
    icon_hsv = get_hsv_stats(icon)
    print(f"Poison Icon Avg HSV: {icon_hsv}")
    
    test_files = [f for f in os.listdir(path) if f.startswith("Poison") and not f.endswith("_icon.png") and f.endswith(".png")]
    ...
    for img_name in test_files:
        ...
        for i, (x, y, w, h) in enumerate(ROIS):
            ...
            # Color match for Poison: Hue ~127, Saturation ~85
            # We use a bit stricter check for Saturation to distinguish from Venom if needed
            color_match = hue_dist < 15 and sat_dist < 40 and area_hsv[1] > 30
            
            # Threshold 75% for shape match
